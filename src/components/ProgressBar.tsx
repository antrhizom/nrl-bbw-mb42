  ...
  return (
    <div className="w-full mb-8">
      ...
      <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden mb-3">
        <div className="h-full bg-bbw-green-500 rounded-full transition-all duration-500" style={{ width: `${progress}%` }} />
      </div>
      <div className="flex gap-0.5">
        {SECTION_IDS.map((id, i) => (
          <div key={id} className={`flex-1 h-1.5 rounded-full transition-colors ${i === currentIndex ? "bg-bbw-green-500" : completedSections.has(id) ? "bg-bbw-green-200" : "bg-gray-200"}`} title={SECTION_LABELS[id]} />
        ))}
      </div>
    </div>
  );
